...
async function run() {
  const RENDER_URL = 'https://mentex-mkii.onrender.com/api';
  const quizTitle = 'JW - Nível 2';
  
  // 1. Fetch quizzes
  const res = await fetch(`${RENDER_URL}/quizzes`);
  const quizzes = await res.json();
  
  let targetQuiz = quizzes.find((q: any) => q.title === quizTitle);
  
  if (!targetQuiz) {
    console.log(`"${quizTitle}" não encontrado no Render. Criando...`);
    const createRes = await fetch(`${RENDER_URL}/quizzes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: quizTitle, description: 'Quiz Bíblico Nível 2 com 50 questões mais detalhadas.' })
    });
    targetQuiz = await createRes.json();
  } else {
    console.log(`Encontrado "${quizTitle}" no Render com ID: ${targetQuiz.id}`);
  }

  // 2. Put the 50 questions
  console.log(`Injetando ${newQuestions.length} questões no quiz ${targetQuiz.id}...`);
  const putRes = await fetch(`${RENDER_URL}/quizzes/${targetQuiz.id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title: quizTitle, questions: newQuestions })
  });

  if (putRes.ok) {
    console.log(`SUCESSO! As ${newQuestions.length} questões foram atualizadas no Render!`);
  } else {
    console.error('Falha ao atualizar:', await putRes.text());
  }
}
