...
async function run() {
  ...
  let targetQuiz = quizzes.find((q: any) => q.title === 'Quiz do Samuca');
  
  if (!targetQuiz) {
    console.log('Quiz do Samuca não encontrado no Render. Criando...');
    const createRes = await fetch(`${RENDER_URL}/quizzes`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title: 'Quiz do Samuca', description: 'Quiz Bíblico com 50 questões' })
    });
    targetQuiz = await createRes.json();
  } else {
    console.log(`Encontrado Quiz do Samuca no Render com ID: ${targetQuiz.id}`);
  }

  // 2. Put the 50 questions
  console.log(`Injetando 50 questões no quiz ${targetQuiz.id}...`);
  const putRes = await fetch(`${RENDER_URL}/quizzes/${targetQuiz.id}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ title: 'Quiz do Samuca', questions: newQuestions })
  });

  if (putRes.ok) {
    console.log('SUCESSO! As 50 questões foram atualizadas no Render!');
  } else {
