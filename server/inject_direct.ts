...
try {
  let targetQuiz = db.prepare('SELECT id FROM Quiz WHERE title = ?').get('Quiz do Samuca');

  if (!targetQuiz) {
    console.log('Quiz do Samuca não encontrado. Criando...');
    const result = db.prepare('INSERT INTO Quiz (title, description, createdAt) VALUES (?, ?, ?)').run('Quiz do Samuca', 'Quiz Bíblico com 50 questões', new Date().toISOString());
    targetQuiz = { id: result.lastInsertRowid };
  } else {
    console.log(`Quiz do Samuca encontrado com ID: ${targetQuiz.id}`);
  }

  // Deleta antigas
  db.prepare('DELETE FROM Question WHERE quizId = ?').run(targetQuiz.id);

  // Insere novas
  const insertStmt = db.prepare('INSERT INTO Question (quizId, text, options, correctIndex, timeLimit) VALUES (?, ?, ?, ?, ?)');
  
  const insertMany = db.transaction((questions) => {
    for (const q of questions) {
      insertStmt.run(targetQuiz.id, q.text, q.options, q.correctIndex, q.timeLimit);
    }
  });

  insertMany(newQuestions);

  console.log(`Sucesso! 50 questões injetadas no Quiz do Samuca (ID: ${targetQuiz.id}).`);
} catch (err) {
  console.error(err);
}
